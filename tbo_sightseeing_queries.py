...
trace_id = -1
def get_attractions_list(country_code, city_id, from_date, to_date, adult_count, child_count,child_age, preferred_currency, keyword=""):
    global trace_id
...
def get_attractions_list_for_multiple_destinations(list_of_jsons):
    ...
    for curr_json in list_of_jsons:
        curr_list = get_attractions_list(curr_json["CountryCode"], curr_json["CityId"], curr_json["FromDate"], curr_json["ToDate"], curr_json["AdultCount"], curr_json["ChildCount"], curr_json["ChildAge"], curr_json["PreferredCurrency"])
        if(curr_list == -1):
            continue
